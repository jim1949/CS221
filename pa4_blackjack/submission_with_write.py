...
class QLearningAlgorithm(util.RLAlgorithm):
    ...
    def incorporateFeedback(self, state, action, reward, newState):
        ...
        residual = self.discount * self.getQ(newState, newAction) + float(reward) - self.getQ(state, action)
        for f, v in self.featureExtractor(state, action):
...
if 0:
    ...
    # QLearningAlgorithm(self, actions, discount, featureExtractor, explorationProb=0.2):
    mdp = smallMDP
    mdp.computeStates()
...
algorithm = QLearningAlgorithm(newThresholdMDP.actions, newThresholdMDP.discount(), identityFeatureExtractor, 0.2)
util.simulate(newThresholdMDP, algorithm)
